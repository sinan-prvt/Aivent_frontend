        ...
        <div className="flex flex-col xl:flex-row h-[calc(100vh-100px)] gap-6">
            <div className="flex-1 flex flex-col space-y-6 overflow-hidden">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <h1 className="text-3xl font-extrabold text-gray-900 font-sans tracking-tight">Decoration Bookings</h1>
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-wrap gap-3 items-center">
                    <div className="relative flex-1 min-w-[200px]">
                        ...
                        <input type="text" placeholder="Search by event or theme..." className="w-full pl-9 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-200 text-sm" />
                    </div>
                </div>
                        ...
                        <>
                            <table className="w-full text-left text-sm">
                                <thead className="bg-gray-50 border-b border-gray-100 text-xs uppercase text-gray-500 font-semibold tracking-wider sticky top-0">
                                    <tr>
                                        <th className="px-6 py-4">Theme/Product</th>
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {bookings.map((booking) => (
                                        <tr
                                            ...
                                            className={`hover:bg-indigo-50/30 transition-colors cursor-pointer ${selectedBooking?.id === booking.id ? 'bg-indigo-50/50' : ''}`}
                                        >
                                            <td className="px-6 py-4 font-medium text-gray-900">
                                                {booking.booking_details?.product_title || 'Decor Item'}
                                            </td>
                                            <td className="px-6 py-4 text-gray-500">
                                                {booking.booking_details?.date || 'N/A'}
                                            </td>
                                            <td className="px-6 py-4 text-gray-900 font-medium">₹{booking.amount}</td>
            ...
            {selectedBooking && (
                <div className="w-full xl:w-96 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 h-full flex flex-col space-y-6 overflow-y-auto">
                    <div>
                        <div className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Order Details</div>
                    ...
                    {selectedBooking.booking_status === "AWAITING_APPROVAL" && (
                        <div className="flex gap-3">
                            <button onClick={handleApprove} className="flex-1 bg-green-600 text-white py-2.5 rounded-xl font-bold text-sm hover:bg-green-700 transition-all flex items-center justify-center gap-2">
                                <FiCheckCircle /> Approve
                            </button>
                            <button onClick={handleReject} className="flex-1 bg-white border border-red-200 text-red-600 py-2.5 rounded-xl font-bold text-sm hover:bg-red-50 transition-all flex items-center justify-center gap-2">
