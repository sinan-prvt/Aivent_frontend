...
export default function SoundSchedule() {
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Manage your gigs, sound checks, and technician assignments.</p>
                </div>
                <div className="flex flex-wrap gap-3">
                    <button
                        onClick={() => handleOpenModal()}
                        className="flex items-center gap-2 px-5 py-3 bg-indigo-600 text-white rounded-xl font-bold shadow-lg hover:bg-indigo-700 transition-all text-sm"
                    >
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                <div className="flex flex-wrap gap-2">
                    <button className="px-4 py-2 bg-indigo-600 text-white rounded-lg font-bold text-xs">
                        All Events
                    </button>
                    <select
                    ...
                    >
                        <option value="all">By Staff/Tech ▾</option>
                        ...
                        <div key={dayIndex} className="border-r border-gray-100 last:border-r-0 p-3 space-y-3 bg-gray-50/30">
                            {filteredTasks
                                ...
                                .map(task => (
                                    ...
                                    >
                                        <div className={`absolute top-0 left-0 w-1 h-full ${task.status === 'confirmed' ? 'bg-indigo-500' :
                                        ...
                                        <div className="flex justify-between items-start gap-2 mb-2">
                                            ...
                                            <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase ${task.status === 'confirmed' ? 'bg-indigo-100 text-indigo-700' :
                                        ...
                                        <div className="mt-3 flex -space-x-2">
                                            {task.assigned_technicians?.map((tech, i) => (
                                                <div
                                                    key={i}
                                                    title={tech}
                                                    className="w-6 h-6 rounded-full bg-indigo-100 border-2 border-white flex items-center justify-center text-[8px] font-black text-indigo-700"
                                                >
                                                    {tech[0]}
                                                </div>
                                            ))}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden scale-in">
                        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                            <h3 className="text-xl font-bold text-gray-900">{editingTask ? "Edit Gig" : "Add New Gig"}</h3>
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="md:col-span-2">
                                    <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Event / Gig Name</label>
                                    <input
                                        type="text"
                                        required
                                        ...
                                        placeholder="e.g. Wedding Reception DJ"
                                    />
                                </div>
                                <div className="md:col-span-2">
                                    <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Location / Venue</label>
                                    <input
                                        type="text"
                                        required
                                ...
                                <div className="md:col-span-2">
                                    <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Assign Staff / Techs</label>
                                    <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto p-4 bg-gray-50 border border-gray-200 rounded-xl">
                                        {technicians.length === 0 ? (
                                            <p className="text-xs text-gray-400 italic col-span-2">No staff profiles found. You can add them later.</p>
                                        ) : (
                                            technicians.map(tech => (
                        ...
                        <div className="p-6 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    onClick={handleSave}
                                    className="flex items-center gap-2 px-8 py-2 bg-indigo-600 text-white rounded-xl font-bold text-sm shadow-lg hover:bg-indigo-700 transition-all"
                                >
                                    <FiCheck /> {editingTask ? "Save Changes" : "Create Gig"}
                                </button>
                            </div>
                        </div>
                    </div>
